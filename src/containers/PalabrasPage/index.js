...
class PalabrasPage extends Component {
  state = {
    loading: false,
    words: []
  }
  ...
  getWords = () => {
    ...
    const url = `${process.env.API_URL}palabras`;
    fetch(url, request)
      .then(response =>
  ...
  render() {
    ...
    return (
      <div className="palabras-page">
        <h3>Palabras</h3>
        <AddWord onAdd={this.addWord} />
        <Filter
          onChange={filter => this.setState({ filter })}
        />
        {words.map(word =>
          <Word word={word} key={word} onDelete={this.removeWord} />)}
      </div>
